 ...
 * {@link org.apache.flink.api.java.typeutils.RowTypeInfo} for representing objects and converts Avro types into types
 * that are compatible with Flink's Table & SQL API.
 *
...
public class AvroSchemaConverter {
    ...
    /**
     * Converts an Avro schema {@code schema} into a nested row structure with deterministic field order and data types
     * that are compatible with Flink's Table & SQL API.
     *
     ...
     *
     * @param schema the schema type, usually it should be the top level record type, e.g. not a nested type
        ...
        switch (logicalType.getTypeRoot()) {
            ...
            case TIMESTAMP_WITHOUT_TIME_ZONE:
                ...
                if (precision <= 3) {
                    ...
                    throw new IllegalArgumentException(
                            "Avro does not support TIMESTAMP type " + "with precision: " + precision
            ...
            case ROW:
                ...
                for (int i = 0; i < rowType.getFieldCount(); i++) {
                    ...
                    SchemaBuilder.GenericDefault<Schema> fieldBuilder = builder.name(fieldName)
    ...
    public static LogicalType extractValueTypeToAvroMap(LogicalType type) {
        ...
        if (!LogicalTypeChecks.hasFamily(keyType, LogicalTypeFamily.CHARACTER_STRING)) {
            throw new UnsupportedOperationException(
                    "Avro format doesn't support non-string as key type of map. " + "The key type is: "
