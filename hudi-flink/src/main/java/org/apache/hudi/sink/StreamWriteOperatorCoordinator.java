...
public class StreamWriteOperatorCoordinator implements OperatorCoordinator {
    ...
    /**
     * Event buffer for one round of checkpointing. When all the elements are non-null and have the same write instant,
     * then the instant succeed and we can commit it.
     */
        ...
        executor.execute(() -> {
            try {
                ...
                result.completeExceptionally(new CompletionException(
                        String.format("Failed to checkpoint Instant %s for source %s", this.instant,
                                this.getClass().getSimpleName()), throwable));
            }
    ...
    private boolean allEventsReceived() {
        return Arrays.stream(eventBuffer).allMatch(event -> event != null && event.isReady(this.instant));
    }
    ...
    private void initInstant(String instant) {
        HoodieTimeline completedTimeline = StreamerUtil.createMetaClient(conf).getActiveTimeline()
    ...
    private void handleWriteMetaEvent(WriteMetadataEvent event) {
        ...
        ValidationUtils.checkState(HoodieTimeline.compareTimestamps(this.instant, HoodieTimeline.GREATER_THAN_OR_EQUALS,
                        event.getInstantTime()),
                String.format("Receive an unexpected event for instant %s from task %d", event.getInstantTime(),
                        event.getTaskID()));
    ...
    /**
     * The coordinator reuses the instant if there is no data for this round of checkpoint, sends the commit ack events
     * to unblock the flushing.
     */
    private void sendCommitAckEvents(long checkpointId) {
        CompletableFuture<?>[] futures = Arrays.stream(this.gateways).filter(Objects::nonNull)
                .map(gw -> gw.sendEvent(CommitAckEvent.getInstance(checkpointId))).toArray(CompletableFuture<?>[]::new);
        try {
    ...
    private static boolean sendToFinishedTasks(Throwable throwable) {
        return throwable.getCause() instanceof TaskNotRunningException || throwable.getCause().getMessage()
                .contains("running");
    }
    ...
    private boolean commitInstant(String instant, long checkpointId) {
        ...
        List<WriteStatus> writeResults = Arrays.stream(eventBuffer).filter(Objects::nonNull)
                .map(WriteMetadataEvent::getWriteStatuses).flatMap(Collection::stream).collect(Collectors.toList());
    ...
    private void doCommit(String instant, List<WriteStatus> writeResults) {
        // commit or rollback
        long totalErrorRecords = writeResults.stream().map(WriteStatus::getTotalErrorRecords).reduce(Long::sum)
                .orElse(0L);
        ...
        if (!hasErrors || this.conf.getBoolean(FlinkOptions.IGNORE_FAILED)) {
            ...
            final Map<String, List<String>> partitionToReplacedFileIds =
                    tableState.isOverwrite ? writeClient.getPartitionToReplacedFileIds(tableState.operationType,
                            writeResults) : Collections.emptyMap();
            ...
            writeResults.stream().filter(WriteStatus::hasErrors).limit(100).forEach(ws -> {
                LOG.error("Global error for partition path {} and fileID {}: {}", ws.getGlobalError(),
    ...
    private static class TableState implements Serializable {
        ...
        private TableState(Configuration conf) {
            ...
            LOG.info("syncHive: " + this.syncHive);
