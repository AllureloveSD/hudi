...
public class HoodieTableFactory implements DynamicTableSourceFactory, DynamicTableSinkFactory {
    ...
    public DynamicTableSource createDynamicTableSource(Context context) {
        ...
        Path path = new Path(conf.getOptional(FlinkOptions.PATH)
                .orElseThrow(() -> new ValidationException("Option [path] should not be empty.")));
        return new HoodieTableSource(schema, path, context.getCatalogTable().getPartitionKeys(),
                conf.getString(FlinkOptions.PARTITION_DEFAULT_NAME), conf);
    }
    ...
    private void sanityCheck(Configuration conf, ResolvedSchema schema) {
        ...
        if (!schema.getPrimaryKey().isPresent()) {
            ...
            if (recordKeys.length == 1 && FlinkOptions.RECORD_KEY_FIELD.defaultValue().equals(recordKeys[0])
            ...
            Arrays.stream(recordKeys).filter(field -> !fields.contains(field)).findAny().ifPresent(f -> {
                throw new HoodieValidationException(
                        "Field '" + f + "' specified in option " + "'" + FlinkOptions.RECORD_KEY_FIELD.key()
        ...
        if (!fields.contains(preCombineField)) {
            if (OptionsResolver.isDefaultHoodieRecordPayloadClazz(conf)) {
                throw new HoodieValidationException(
                        "Option '" + FlinkOptions.PRECOMBINE_FIELD.key() + "' is required for payload class: "
            ...
            if (preCombineField.equals(FlinkOptions.PRECOMBINE_FIELD.defaultValue())) {
                ...
                throw new HoodieValidationException(
                        "Field " + preCombineField + " does not exist in the table schema." + "Please check '"
     ...
     */
    private static void setupConfOptions(Configuration conf, String tableName, CatalogTable table,
    ...
    private static void setupHoodieKeyOptions(Configuration conf, CatalogTable table) {
        List<String> pkColumns = table.getSchema().getPrimaryKey().map(UniqueConstraint::getColumns)
    ...
    public static void setupTimestampKeygenOptions(Configuration conf, DataType fieldType) {
        ...
        if (DataTypeUtils.isTimestampType(fieldType)) {
            ...
            String partitionFormat = conf.getOptional(FlinkOptions.PARTITION_FORMAT)
    ...
    private static void setupHiveOptions(Configuration conf) {
        if (!conf.getBoolean(FlinkOptions.HIVE_STYLE_PARTITIONING) && FlinkOptions.isDefaultValueDefined(conf,
    ...
    private static void setupReadOptions(Configuration conf) {
        if (!conf.getBoolean(FlinkOptions.READ_AS_STREAMING) && (
                conf.getOptional(FlinkOptions.READ_START_COMMIT).isPresent() || conf.getOptional(
                        FlinkOptions.READ_END_COMMIT).isPresent())) {
     ...
     * {@link FlinkOptions#SOURCE_AVRO_SCHEMA_PATH} and {@link FlinkOptions#SOURCE_AVRO_SCHEMA} are not specified.
     *
     * @param conf    The configuration
     * @param rowType The specified table row type
     */
    private static void inferAvroSchema(Configuration conf, org.apache.flink.table.types.logical.LogicalType rowType) {
        if (!conf.getOptional(FlinkOptions.SOURCE_AVRO_SCHEMA_PATH).isPresent() && !conf.getOptional(
                FlinkOptions.SOURCE_AVRO_SCHEMA).isPresent()) {
